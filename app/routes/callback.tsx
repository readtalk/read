...
export default function Callback() {
    ...
    const exchangeCode = async () => {
      try {
        ...
        if (data.access_token) {
          ...
          if (data.refresh_token) localStorage.setItem("readtalk_refresh_token", data.refresh_token);
          if (data.expires_in) localStorage.setItem("readtalk_token_expiry", Date.now() + data.expires_in * 1000);
          
          sessionStorage.removeItem("oauth_state");
          
          setStatus("success");
          setTimeout(() => navigate("/resendlist"), 1000);
        } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[100dvh] bg-white dark:bg-gray-950 px-4">
      ...
      <button onClick={() => navigate("/")} className="mt-6 px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
        Back to Home
      </button>
    </div>
  );
